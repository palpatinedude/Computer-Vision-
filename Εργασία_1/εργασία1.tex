...
\begin{itemize}
    \item Κάθε pixel παίρνει μια ετικέτα (π.χ. δρόμος, αυτοκίνητο, άνθρωπος, κτίριο).
    \item Η εικόνα χωρίζεται σε περιοχές που αντιστοιχούν σε διαφορετικά αντικείμενα ή κλάσεις.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Δέχεται χάρτες χαρακτηριστικών από τα προηγούμενα συνελικτικά επίπεδα και εφαρμόζει pooling σε διαφορετικά επίπεδα πλέγματος .
        \item Κωδικοποιεί χωρική πληροφορία σε πολλαπλές κλίμακες χωρίς να χρειάζεται να αλλάξεις το μέγεθος εισόδου εικόνας.
    \end{itemize}
    
    \item Pyramid Pooling Module:
    \begin{itemize}
        \item Συγχωνεύει τα χαρακτηριστικά από διαφορετικές κλίμακες για να ενισχύσει την αντίληψη του πλαισίου της σκηνής.
        \item Επιτρέπει στο μοντέλο να "βλέπει" τόσο global (π.χ. ολόκληρος δρόμος) όσο και τοπικές λεπτομέρειες (π.χ. μικρές πινακίδες ή άνθρωποι).
    \end{itemize}
\end{enumerate}   

\subsection{Στόχος Αξιολόγησης}
...
\] 
όπου \(TP_c\) είναι τα \textit{true positives}, \(FP_c\) τα \textit{false positives} και \(FN_c\) τα \textit{false negatives} για την κλάση \(c\). Η IoU μετράει την επικάλυψη μεταξύ των προβλέψεων και των πραγματικών ετικετών για κάθε κλάση.
...
\] 
όπου \(N_\text{classes}\) είναι ο αριθμός των κλάσεων. Το mIoU δείχνει την συνολική επίδοση του μοντέλου σε όλες τις κλάσεις.

\paragraph{Pixel Accuracy} 
\mbox{}\\
Η \textit{Pixel Accuracy} ορίζεται ως
\[
